...
export function csvExport(s:State,view:string,query='',filter='all',companyFilter='all'){
 ...
 }else if(view==='Proyectos')rows=[['Proyecto','Empresa','Pedido EUR','Coste horas EUR','Margen EUR'],...s.projects.filter(p=>scoped(companyForOrder(s,p.orderId))&&match(p.title,company(companyForOrder(s,p.orderId))?.name)).map(p=>{const o=s.orders.find(o=>o.id===p.orderId)!;return[p.title,company(companyForOrder(s,o.id))?.name,total(o.lines),projectCost(s,p.id),total(o.lines)-projectCost(s,p.id)]})];
