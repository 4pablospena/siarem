import { stages, today, dateOffset, risk, total, projectCost, companyForOrder, eur, ensureState, type State } from './crm.ts';
...
export function businessReport(input: State, now = today()): BusinessReport {
  ...
  const funnel = stages.filter(stage => stage !== 'Ganada' && stage !== 'Perdida').map(stage => {
    const rows = open.filter(o => o.stage === stage);
    return { stage, count: rows.length, amount: rows.reduce((sum, o) => sum + o.amount, 0) };
  });
  const closedInPeriod = (stage: 'Ganada' | 'Perdida') =>
    s.opportunities.filter(o => o.stage === stage && o.closeDate >= periodStart && o.closeDate <= periodEnd);
  const wonRows = closedInPeriod('Ganada');
  const lostRows = closedInPeriod('Perdida');
