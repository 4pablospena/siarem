...
test('server: authentication, tenant isolation, team membership, conflict handling and persistence',async()=>{
 ...
 let response=await request('a',{action:'save',kind:'companies',record:{id:'secret-a',name:'Visible only to A',contact:'',email:'',phone:'',contactDays: 45, taxId: '', address: '', paymentDays: 30},revision:0});assert.equal(response.status,200);
 ...
 response=await request('colleague',{action:'save',kind:'companies',record:{id:'colleague-c',name:'Created by colleague',contact:'',email:'',phone:'',contactDays: 30, taxId: '', address: '', paymentDays: 30},revision:joined.body.revision});assert.equal(response.status,200);
 const persisted=(await request('a')).body;assert.equal(persisted.state.companies.some(c=>c.id==='colleague-c'),true);
 const [write1,write2]=await Promise.all(['one','two'].map(n=>request('a',{action:'save',kind:'companies',record:{id:n,name:n,contact:'',email:'',phone:'',contactDays: 30, taxId: '', address: '', paymentDays: 30},revision:persisted.revision})));
