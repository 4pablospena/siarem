...
test('follow-up draft names the same next action as the record, never the placeholder', async () => {
  ...
  const state = emptyState();
  state.companies = [{ id: 'company', demo: false, name: 'Acme', contact: 'Ana', email: '', phone: '', contactDays: 30, taxId: '', address: '', paymentDays: 30, companyRole: 'client' }];
...
test('Siarem-bot brief lists only open work, with the shared next action', async () => {
  ...
  const state = emptyState();
  state.companies = [{ id: 'company', demo: false, name: 'Acme', contact: '', email: '', phone: '', contactDays: 30, taxId: '', address: '', paymentDays: 30, companyRole: 'client' }];
...
test('areas map each commercial view and open on their first tab', async () => {
  ...
  assert.deepEqual(areas.find(area => area.name === 'OCR')?.views, ['OCR']);
  assert.equal(areaOf('OCR')?.name, 'OCR');
  assert.ok(navigateTargets().includes('Pipeline'));
  assert.ok(navigateTargets().includes('Proyectos'));
  assert.ok(navigateTargets().includes('OCR'));
