...
test('ensureState migrates legacy stage names to stageId and installs default map', () => {
  const raw = {
    ...
    companies: [{ id: 'c', demo: false, name: 'Acme', contact: '', email: '', phone: '', contactDays: 30, taxId: '', address: '', paymentDays: 30, companyRole: 'client' }],
  } as never;
...
test('WIP block prevents entering a full column; rank swaps order', () => {
  ...
  state.companies.push({ id: 'c', demo: false, name: 'Acme', contact: '', email: '', phone: '', contactDays: 30, taxId: '', address: '', paymentDays: 30, companyRole: 'client' });
